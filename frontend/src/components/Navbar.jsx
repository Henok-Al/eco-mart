        ...
        <div className="flex flex-wrap justify-between items-center">
          ...
          <nav className="flex flex-wrap items-center gap-4">
            ...
            {user && (
              <Link
                to={"/cart"}
                className="relative group text-gray-600 hover:text-gray-800 transition duration-300 ease-in-out"
              >
                <ShoppingCart
                  className="inline-block mr-1 group-hover:text-gray-800"
                  size={20}
                />
                <span className="hidden sm:inline">Cart</span>
                {cart.length > 0 && (
                  <span className="absolute -top-2 -right-2 bg-gray-500 text-white rounded-full px-2 py-0.5 text-xs group-hover:bg-gray-600 transition duration-300 ease-in-out">
                    {cart.length}
                  </span>
                )}
              </Link>
